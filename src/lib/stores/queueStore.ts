...
export function findShortestQueue(machineIds: string[]): string | null {
	...
	for (const machineId of machineIds) {
		...
		const queueLength = queue ? queue.sessions.filter(s => s.status === 'WAITING' || s.status === 'ACTIVE').length : 0;
...
export function occupySlot(slotId: string, machineIds: string[], sessionMode: SessionMode = 'SOLO', partnerSlotId?: string): boolean {
		...
		queues.update(q => {
			return q.map(queue => {
				...
				const openSession = queue.sessions.find(
					s => s.status === 'WAITING' && s.mode === 'OPEN' && s.playerSlots.length < s.capacity
				);
		...
		queues.update(q => {
			return q.map(queue => {
				if (queue.machineId === bestMachineId) {
					...
					const activeCount = updatedSessions.filter(s => s.status === 'ACTIVE').length;
		...
		return q.map(queue => {
			const updatedSessions = queue.sessions
				.map(session => {
					...
					const filteredSlots = session.playerSlots.filter(slot => slot !== slotId);
					
					if (filteredSlots.length !== session.playerSlots.length) {
						// Player was in this session
						if (filteredSlots.length === 0) {
							// Session is now empty, mark as COMPLETED
							return { ...session, status: 'COMPLETED' as SessionStatus, playerSlots: [] };
						} else {
...
export function moveToEndOfQueue(slotId: string, currentMachineId: string) {
	...
	for (const queue of allQueues) {
		const session = queue.sessions.find(s => 
	...
	queues.update(q => {
		return q.map(queue => {
			if (queue.machineId === currentMachineId) {
				...
				const filteredSessions = queue.sessions.filter(s => s.id !== playerSession!.id);
				
				// Promote next session to ACTIVE
				if (filteredSessions.length > 0 && !filteredSessions.some(s => s.status === 'ACTIVE')) {
...
export function updateSessionMode(sessionId: string, machineId: string, newMode: SessionMode) {
	queues.update(q => {
		return q.map(queue => {
		...
		return q.map(queue => {
			if (queue.machineId === machineId) {
				return {
					...queue,
					sessions: queue.sessions.map(session => {
						if (session.id === sessionId && session.mode === 'OPEN' && session.playerSlots.length === 1) {
...
export const queueStats = derived(queues, $queues => {
	return $queues.map(queue => ({
		...
		waitingSessions: queue.sessions.filter(s => s.status === 'WAITING').length,
		totalPlayers: queue.sessions.reduce((sum, s) => sum + s.playerSlots.length, 0)
	}));
});
