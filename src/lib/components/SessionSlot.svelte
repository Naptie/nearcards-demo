...
<div
  ...
  title={session.status === "ACTIVE"
    ? "Long press to re-queue | Double-click to edit"
    : "In queue | Double-click to edit"}
>
  <!-- Player slots -->
  <div class="flex flex-wrap gap-1 justify-center mb-2">
    {#each session.playerSlots as slotId}
      <div
        class="px-2 py-1 text-sm font-bold text-white bg-black bg-opacity-30 rounded"
      >
        {slotId}
      </div>
    {/each}
  ...
  <div
    class="absolute top-2 right-2 text-xs bg-black bg-opacity-50 px-2 py-1 rounded"
  >
  ...
  <!-- Capacity indicator -->
  <div
    class="absolute bottom-2 left-2 text-xs bg-black bg-opacity-50 px-2 py-1 rounded"
  >
  ...
  {#if session.status === "ACTIVE"}
    <div
      class="absolute top-2 left-2 w-3 h-3 rounded-full bg-white animate-blink"
    ></div>
  {/if}
</div>
