...
>
  <div class="card-body p-6">
    <div class="flex items-center gap-4 mb-4">
      <!-- Machine Icon -->
      <div
        class="w-16 h-16 flex items-center justify-center text-4xl bg-primary rounded-2xl shadow-lg"
      >
        {machine.icon}
      </div>
      ...
      <div class="flex-1">
        ...
        <div class="text-sm opacity-70">
          {queue.sessions.length} session{queue.sessions.length !== 1
            ? "s"
            : ""}
          {#if machine.seats > 1}
            <span class="badge badge-accent badge-sm ml-2"
              >{machine.seats} seats</span
            >
          {/if}
        </div>
      </div>
    ...
    {#if queue.sessions.length === 0}
      <div class="text-center text-base-content/50 italic py-8">
