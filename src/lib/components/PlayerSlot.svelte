...
<div
  class="relative flex flex-col items-center justify-center w-20 h-20 rounded-xl cursor-pointer select-none transition-all duration-300 {player.status ===
  'playing'
    ? 'bg-linear-to-br from-pink-500 to-rose-600 shadow-lg shadow-pink-500/50 animate-pulse-playing'
    : 'bg-linear-to-br from-purple-600 to-indigo-700 shadow-md'} {pressing
    ? 'scale-95'
    : 'hover:scale-110'} animate-slide-in"
  ...
  role="button"
  tabindex="0"
  title={player.status === "playing"
    ? "Long press to move to end of queue"
    : "In queue"}
>
  ...
  <div
    class="absolute top-2 right-2 w-3 h-3 rounded-full bg-white/80 {player.status ===
    'playing'
      ? 'animate-blink'
      : ''}"
  ></div>
